# -*- coding: utf-8 -*-
"""Cleaned and Overfitting-Reduced Pregnancy Risk Prediction Model"""

# !pip install catboost imbalanced-learn scikit-learn
...
# Fill missing values
df['Supplements'] = df['Supplements'].fillna('magnesium')
df['Symptoms'] = df['Symptoms'].fillna('dizziness')

# Drop unwanted columns
if 'Trimester' in df.columns:
    df = df.drop(columns=['Trimester'])
...
# Target creation with reduced noise
def create_multiclass_output_with_noise(row):
    score = calculate_risk_score(row)
    if random.random() < 0.15:  # Reduced noise to prevent overfitting
...
temp = temp.drop(columns=['Currently_Pregnant'])

# Train-test split
train_df, test_df = train_test_split(temp, test_size=0.2, random_state=42, stratify=temp['Pregnancy_Risk_Level'])

# Binary encoding
binary_cols = ['Use_Fortified_Food', 'Smoke_Alcohol', 'Clean_Water', 'Food_Access_Difficulty']
...
for col in cat_cols:
    if col in train_df.columns:
        ...
        train_df[f'{col}_te'] = train_df[f'{col}_te'].fillna(train_df[target_col].mean())
        test_means = train_df.groupby(col)[target_col].mean()
        test_df[f'{col}_te'] = test_df[col].map(test_means)
        test_df[f'{col}_te'] = test_df[f'{col}_te'].fillna(train_df[target_col].mean())
...
# SMOTE Oversampling with smaller k_neighbors to avoid errors
